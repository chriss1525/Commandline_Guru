...
# load question-answering model from transformes
linux_model_name = "openai/linux-gpt"
github_model_name = "openai/github-gpt"

linux_qa_model = pipeline("question-answering", model=linux_model_name)
github_qa_model = pipeline("question-answering", model=github_model_name)
...
def reply_to_text(update: Update, context: CallbackContext):
    ...
    # Check if the text is a Linux command question
    linux_answer = linux_qa_model({
        "question": text,
        "context": "Linux commands are used to interact with the Linux operating system.",
    })
    if linux_answer["score"] > 0.5:
        update.message.reply_text(linux_answer["answer"])
        return

    # Check if the text is a GitHub command question
    github_answer = github_qa_model({
        "question": text,
        "context": "GitHub commands are used to interact with GitHub repositories.",
    })
    if github_answer["score"] > 0.5:
        update.message.reply_text(github_answer["answer"])
        return
    # Check if the text contains a Linux command
    linux_command = re.search(r"\b(linux|terminal|command|shell)\b", text)
    if linux_command:
        update.message.reply_text(
            "It seems like you mentioned a Linux command. If you have any specific questions, feel free to ask!")
        return

    # Check if the text contains a GitHub command
    github_command = re.search(
        r"\b(github|repository|commit|pull request|branch)\b", text)
    if github_command:
        update.message.reply_text(
            "It looks like you mentioned a GitHub command. If you need any help with GitHub, let me know!")
        return
...
updater.dispatcher.add_handler(MessageHandler(Filters.text("thank you"), thank_you))
